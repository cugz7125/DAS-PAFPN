...
class ConvolutionLayer(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=1, stride=1, padding=None,
                 groups=1, dilation=1, activation=True, norm_cfg=dict(type='BN')):
        """初始化卷积层"""
        super().__init__()
        # 创建卷积层，自动计算填充
        ...
        if norm_cfg['type'] == 'BN':
            ...
            self.batch_norm = nn.SyncBatchNorm(out_channels)
        else:
            self.batch_norm = nn.BatchNorm2d(out_channels)
...
class MKPConv(nn.Module):
    ...
    def __init__(self, channels, kernel_size=1, stride=1, padding=None, 
        ...
        self.pointwise_conv_3x3 = ConvolutionLayer(channels, channels, kernel_size=1, 
        ...
        self.pointwise_conv_5x5 = ConvolutionLayer(channels, channels, kernel_size=1, 
                                                  stride=1, norm_cfg=norm_cfg)
        ...
        self.pointwise_conv_7x7 = ConvolutionLayer(channels, channels, kernel_size=1, 
                                                  stride=1, norm_cfg=norm_cfg)
